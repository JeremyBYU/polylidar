...
public:
    ...
    HalfEdgeTriangulation(Matrix<double> &in_coords);
...
{
public:
    Matrix<double> triangle_normals_matrix;
    ...
    TriMesh(const Matrix<double> &in_coords, std::vector<size_t> &in_triangles, std::vector<size_t> &in_halfedges, const bool copy_vertices=true);
    TriMesh();
